...
class TimeEmbedding(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = F.silu(self.linear_1(x))  # (1, 1280)
        x = self.linear_2(x)
...
class UNET_AttentionBlock(nn.Module):
    ...
    def forward(self, x, context):
        ...
        # Normalization + Feed-Forward Network with skip connection
        residue_short = x
        ...
        x += residue_short  # (Batch_Size, Height*Width, Features)


        x = x.transpose(-1, -2)  # (Batch_Size, Features, Height*Width)

        x = x.view((n, c, h, w))

        return self.conv_output(x) + residue_long  # (Batch_Size, Features, Height, Width)
...
class UNET(nn.Module):
    ...
    def __init__(self):
        ...
        self.decoders = nn.ModuleList([
            ...
            # (Batch_Size, 1280, Height/16, Width/16) - > (Batch_Size, 640, Height/8, Width/8)
...
class UNET_OutputLayer(nn.Module):
    def __init__(self, in_channels: int, out_channels: int):
        super().__init__()
        self.growpnorm = nn.GroupNorm(32, in_channels)
...
class Diffusion(nn.Module):
    ...
    def forward(self, latent: torch.Tensor, context: torch.Tensor, time: torch.Tensor):
        ...
        time = self.time_embedding(time)  # (1, 320)

        output = self.unet(latent, context, time)  # (Batch_Size, 320, Height/8, Width/8)

        output = self.final(output)  # (Batch_Size, 4, Height, Width)
